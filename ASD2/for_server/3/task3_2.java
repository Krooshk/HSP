...
class BSTAdditional<T>
{
    ...
    // Задание 1 WideAllNodes, Time - O(N), Space - O(1)
    ...
    // Задание 2 DeepAllNodes, Time - O(N), Space - O(1)
    ArrayList<BSTNode> DeepAllNodes(int order) {
        return traverseForDeep(Root, order);
    }

    private ArrayList<BSTNode> traverseForDeep(BSTNode<T> node, int order) {
        ArrayList<BSTNode> list = new ArrayList<>();
        if (node == null) {
            return list;
        }
        ...
        if (order == 0) {
            list.addAll(traverseForDeep(node.LeftChild, order));
            list.add(node);
            list.addAll(traverseForDeep(node.RightChild, order));
        }
    ...
    // Задание 3* invert, Time - O(N), Space - O(1)
    BSTAdditional invert() {
        traverseInvert(Root);

        return this;
    }

    void traverseInvert(BSTNode node) {
        if (node == null) return;
    ...
    // Задание 4* getMaximumSumLevel, Time - O(N), Space - O(n)
    // Как вариант, хранить общую сумму для уровня в мапе, а рекурсивно передавать мапу, мапа передаётся по ссылке.
    Integer getMaximumSumLevel() {
        HashMap<Integer, Integer> map = new HashMap<>();
        if (Root == null) return null;
        helperForMaximumSum((BSTNode<Integer>) Root, map, 0);
    ...
    void helperForMaximumSum(BSTNode<Integer> node, HashMap<Integer, Integer> map, int level) {
        if (node == null) {
            return;
        }
        int newValue = map.get(level) == null ? node.NodeValue : map.get(level) + node.NodeValue;
        map.put(level, newValue);

        helperForMaximumSum(node.LeftChild, map, level + 1);
        helperForMaximumSum(node.RightChild, map, level + 1);
    }

    // Задание 5* getBst, Time - O(N), Space - O(n)
    //префиксный массив: [1,2,4,5,3,6,7] - тут мы знаем только то, что первый элемент это корень дерева, а как дальше разобьются значения - непонятно.
    // Если отбросить значения, у нас может быть слева от корня 2,4,5,3,6, а справа 7. Либо слева 2,4,5,3,6,7, а справа ничего и друшие варианты.
    // Второй массив дает возможность построить полную картину дерева с уверенностью в правильности.
