...
class AdditonalGraph
{
    ...
    // Task 2* getMaxRemoteNode, Time - O(N^3), Space - O(N)
    int getMaxRemoteNode(){
        int result = 0;
        for (int i = 0; i  < vertex.length - 1; i++ ) {
            for (int j = i + 1; j < vertex.length; j++) {
                int path = BreadthFirstSearch(i, j).size() - 1;
    ...
    private void helperBFSCycles(Integer start, ArrayList<Integer> queue, HashMap<Integer, Integer> map, HashSet store){

        int[] currentV = m_adjacency[start];
        for (int i = 0; i < currentV.length; i++) {
            boolean isSuit = currentV[i] == 1 && !vertex[i].Hit;
            boolean isParent = map.get(start) != null && map.get(start) == i;
            boolean isCycle = currentV[i] == 1 && vertex[i].Hit && !isParent;
            if (isSuit) {
                vertex[i].Hit = true;
                queue.addLast(i);
                map.put(i, start);
            ...
            if (isCycle) {
                ...
                list.add(i);
                list.add(start);
                getIndexesHelper(start, i, list, map);
        ...
        Integer elem = queue.removeFirst();
        helperBFSCycles(elem, queue, map, store);
    }

    private void getIndexesHelper(int start, int current, ArrayList<Integer> list, HashMap<Integer, Integer> map){
        Integer parent = map.get(current);
        ...
        list.add(parent);
        if (parent != start) {
            getIndexesHelper(start, parent, list, map);
        }
    }

    // Task 3* getCycles, Time - O(N^2), Space - O(N)
    ArrayList<ArrayList<Integer>> getCycles(){
        for (int i = 0; i < vertex.length; i++){
            if (vertex[i] != null) vertex[i].Hit = false;
        }
        vertex[0].Hit = true;
        ...
        helperBFSCycles(0, queue, map, store);

        ArrayList<ArrayList<Integer>> result = new ArrayList<>();
        for (ArrayList<Integer> el: store) {
            result.add(el);
        }
        return result;
    }
